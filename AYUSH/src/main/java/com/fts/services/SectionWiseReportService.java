...
@Service
public class SectionWiseReportService implements GridComponent,ComboComponent{
	private static final Log LOG = LogFactory.getLog(SectionWiseReportService.class);
	@Autowired
		...
		try {
			...
			if(extraParams[0]!=null)
			{
				if(extraParams[0].contains(","))
				{
					...
					fileEntryList = fileEntryManager.getAllActiveFilesForSectionWiseReport(sortInfo,filterString,start,limit,extraParams,sectionId,fromDate,toDate);
					gridDto.setTotal(fileEntryManager.getAllActiveFilesForSectionWiseReportCount(filterString,extraParams,sectionId,fromDate,toDate));
				}
			}
			else
			{
