...
@Service
public class StatusWiseReportService implements GridComponent,ComboComponent{
	private static final Log LOG = LogFactory.getLog(StatusWiseReportService.class);
	@Autowired
		...
		try {
			...
			if(extraParams[0]!=null)
			{
				if(extraParams[0].contains(","))
				{
					...
					long fileStatusId	=Long.parseLong(param[0]);
					String fromDate	=	param[1];
					String toDate	=	param[2];
					fileEntryList = fileEntryManager.getAllActiveFilesForStatusWiseReport(sortInfo,filterString,start,limit,extraParams,fileStatusId,fromDate,toDate);
					gridDto.setTotal(fileEntryManager.getAllActiveFilesForStatusWiseReportCount(filterString,extraParams,fileStatusId,fromDate,toDate));
				}
			}
			else
			{
